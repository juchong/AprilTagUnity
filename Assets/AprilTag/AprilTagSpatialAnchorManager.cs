...
namespace AprilTag
{
    ...
    public class AprilTagSpatialAnchorManager : MonoBehaviour
    {
        ...
        [Range(0.0f, 1.0f)]
        [SerializeField]
        private float minConfidenceThreshold = 0.3f; // Lowered for easier anchor placement

        [Tooltip("Number of consecutive high-confidence detections required before placing anchor")]
        [SerializeField]
        private int requiredStableFrames = 2; // Reduced for easier placement

        [Tooltip("Maximum time to wait for stable detection before giving up (seconds)")]
        [SerializeField]
        private float maxDetectionTimeout = 30f; // Increased for better success rate

        [Tooltip("Enable anchor persistence across app sessions")]
        [SerializeField]
        private bool persistAnchors = true;
        ...
        [SerializeField]
        private bool enableKeepOutZone = true; // Re-enabled with appropriate settings for 16.5cm tags
        ...
        private Dictionary<int, global::OVRSpatialAnchor> _anchorsById =
            new Dictionary<int, global::OVRSpatialAnchor>();
        private Dictionary<int, AnchorPlacementState> _placementStates =
            new Dictionary<int, AnchorPlacementState>();
        ...
        void Start()
        {
            ...
            {
                Debug.Log(
                    $"[AprilTagSpatialAnchorManager] Initialized with confidence threshold: {minConfidenceThreshold:F3}, required stable frames: {requiredStableFrames}, timeout: {maxDetectionTimeout:F1}s"
                );
            }
        }
        ...
        private void InitializeSpatialAnchorCore()
        {
            ...
            if (spatialAnchorCore != null)
            {
                ...
                spatialAnchorCore.OnAnchorCreateCompleted.RemoveListener(
                ...
                // Subscribe to events
                spatialAnchorCore.OnAnchorCreateCompleted.AddListener(
                    OnAnchorCreatedFromBuildingBlock
                );
                ...
                if (enableDebugLogging)
                {
                    ...
                    Debug.Log(
                        $"[AprilTagSpatialAnchorManager] SpatialAnchorCore GameObject: {spatialAnchorCore.gameObject.name}"
                    );
                    Debug.Log(
                        $"[AprilTagSpatialAnchorManager] SpatialAnchorCore active: {spatialAnchorCore.gameObject.activeInHierarchy}"
                    );
                }
            }
            else
        ...
        {
            if (!enableSpatialAnchors || _isClearingAnchors)
                return;
            ...
            {
                Debug.Log(
                    $"[AprilTagSpatialAnchorManager] Processing tag {tagId} with confidence {confidence:F3}, threshold: {minConfidenceThreshold:F3}"
                );
                Debug.Log(
                    $"[AprilTagSpatialAnchorManager] Currently tracking {_anchorsById.Count} anchors: [{string.Join(", ", _anchorsById.Keys)}]"
                );
            }
            ...
            if (confidence >= minConfidenceThreshold)
            {
                ...
                {
                    Debug.Log(
                        $"[AprilTagSpatialAnchorManager] Tag {tagId}: {state.stableFrameCount}/{requiredStableFrames} stable frames, confidence: {confidence:F3} (threshold: {minConfidenceThreshold:F3})"
                    );
                }
            }
            else
            {
                ...
                {
                    Debug.Log(
                        $"[AprilTagSpatialAnchorManager] Tag {tagId}: Confidence dropped below threshold ({confidence:F3} < {minConfidenceThreshold:F3}), resetting stable frames"
                    );
                }
            ...
            ) // Log every 2 seconds
            {
                ...
                Debug.Log(
                    $"[AprilTagSpatialAnchorManager] Tag {tagId}: Timeout in {timeRemaining:F1}s, frames: {state.stableFrameCount}/{requiredStableFrames}, confidence: {confidence:F3}"
                );
            }
            ...
            )
            {
                ...
                if (IsPositionInKeepOutZone(position, tagId))
                {
                    if (enableDebugLogging)
                    {
                        ...
                        Debug.Log(
                            $"[AprilTagSpatialAnchorManager] Current keep out zones: {_keepOutZones.Count}"
                        );
        ...
        )
        {
            ...
            if (enableDebugLogging)
            {
                ...
                Debug.Log(
                    $"[AprilTagSpatialAnchorManager] Total anchors tracked: {_anchorsById.Count}"
                );
            }
        ...
        private void FallbackClearAllAnchors()
        {
            ...
            var allSpatialAnchors = FindObjectsByType<global::OVRSpatialAnchor>(
        ...
        private void HandleClearAnchorsInput()
        {
            ...
            if (aButtonPressed && !_lastAButtonState)
            {
                ...
                {
                    Debug.Log(
                        $"[AprilTagSpatialAnchorManager] A button pressed - clearing all anchors (currently tracking {_anchorsById.Count} anchors)"
                    );
                }
                ClearAllAnchors();
            }
        ...
        private void OnAnchorCreatedFromBuildingBlock(
            global::OVRSpatialAnchor anchor,
            global::OVRSpatialAnchor.OperationResult result
        )
        {
        ...
        {
            if (!persistAnchors || spatialAnchorCore == null)
                return;
