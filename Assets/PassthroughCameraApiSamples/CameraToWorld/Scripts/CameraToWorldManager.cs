...
namespace PassthroughCameraSamples.CameraToWorld
{
    ...
    public class CameraToWorldManager : MonoBehaviour
    {
        ...
        [SerializeField]
        private GameObject m_centerEyeAnchor;

        [SerializeField]
        private GameObject m_headMarker;

        [SerializeField]
        private GameObject m_cameraMarker;

        [SerializeField]
        private GameObject m_rayMarker;

        [SerializeField]
        private CameraToWorldCameraCanvas m_cameraCanvas;

        [SerializeField]
        private float m_canvasDistance = 1f;

        [SerializeField]
        private Vector3 m_headSpaceDebugShift = new(0, -.15f, .4f);
        private GameObject m_rayGo1,
            m_rayGo2,
            m_rayGo3,
            m_rayGo4;
        ...
        private IEnumerator Start()
        {
            ...
            {
                Debug.LogError(
                    $"PCA: {nameof(m_webCamTextureManager)} field is required "
            ...
            m_webCamTextureManager.RequestedResolution = PassthroughCameraUtils
                .GetCameraIntrinsics(CameraEye)
                .Resolution;
        ...
        private void Update()
        {
            ...
            if (OVRInput.GetDown(OVRInput.Button.Two))
            {
                m_isDebugOn ^= true;
                Debug.Log(
                    $"PCA: SpatialSnapshotManager: DEBUG mode is {(m_isDebugOn ? "ON" : "OFF")}"
                );
                UpdateRaysRendering();
        ...
        private void ScaleCameraCanvas()
        {
            ...
            var leftSidePointInCamera = PassthroughCameraUtils.ScreenPointToRayInCamera(
                CameraEye,
                new Vector2Int(0, CameraResolution.y / 2)
            );
            var rightSidePointInCamera = PassthroughCameraUtils.ScreenPointToRayInCamera(
                CameraEye,
                new Vector2Int(CameraResolution.x, CameraResolution.y / 2)
            );
            var horizontalFoVDegrees = Vector3.Angle(
                leftSidePointInCamera.direction,
                rightSidePointInCamera.direction
            );
            ...
            var localScale = (float)(
                newCanvasWidthInMeters / cameraCanvasRectTransform.sizeDelta.x
            );
        ...
        private void UpdateRaysRendering()
        {
            ...
            {
                rayGo
                    .GetComponent<CameraToWorldRayRenderer>()
                    .RenderMiddleSegment(m_snapshotTaken || m_isDebugOn);
            }
        }

        private void UpdateMarkerPoses()
        {
            var headPose = OVRPlugin
            ...
            var rays = new[]
            {
                ...
                new
                {
                    rayGo = m_rayGo2,
                    u = 0,
                    v = CameraResolution.y,
                },
                new
                {
                    rayGo = m_rayGo3,
                    u = CameraResolution.x,
                    v = CameraResolution.y,
                },
                new
                {
            ...
            {
                var rayInWorld = PassthroughCameraUtils.ScreenPointToRayInWorld(
                    CameraEye,
                    new Vector2Int(item.u, item.v)
                );
                item.rayGo.transform.position = rayInWorld.origin;
                item.rayGo.transform.LookAt(rayInWorld.origin + rayInWorld.direction);

                var angleWithCameraForwardDegree = Vector3.Angle(
                    item.rayGo.transform.forward,
                ...
                var zScale = (float)(
                    m_canvasDistance / Math.Cos(angleWithCameraForwardDegree / 180 * Math.PI) / 0.5
                );
                item.rayGo.transform.localScale = new Vector3(
                    item.rayGo.transform.localScale.x,
                    item.rayGo.transform.localScale.y,
                    zScale
                );
        ...
        private void TranslateMarkersForDebug(bool moveForward)
        {
            ...
            var direction = m_snapshotTaken
                ? m_snapshotHeadPose.orientation
                : m_centerEyeAnchor.transform.rotation;
